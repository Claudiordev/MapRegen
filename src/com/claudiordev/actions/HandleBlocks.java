...
public class HandleBlocks implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        synchronized (BlockProcessor.blocks) {
            BlockProcessor.blocks.add(new DataBlock(event.getBlock(),"BREAK",event.getPlayer().getDisplayName()));
    ...
    public void onBlockExplode(final EntityExplodeEvent event) {
        ...
        for (int i = 0; i < event.blockList().size(); i++) {
            blocks.add(new DataBlock(event.blockList().get(i),"BREAK","SERVER"));
        }
